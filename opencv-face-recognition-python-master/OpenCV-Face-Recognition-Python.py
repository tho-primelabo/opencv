...
subjects = ["", "Ramiz Raja", "Elvis Presley", "Tho Le", "my-face"]
...
def predict(test_img):
    ...
    #img = cv2.resize(test_img, (400, 500))
    #gray = cv2.cvtColor(test_img, cv2.COLOR_BGR2GRAY)
    face, rect = detect_face(img)
    #face_resize = cv2.resize(face, (400, 500))
    #predict the image using our face recognizer
    label, confidence = face_recognizer.predict(face)
    print(confidence)
    if(confidence != 0.0):
      label_text = "not recognizer"
...
#res = cv2.resize(cap.read()[0],None,fx=2, fy=2, interpolation = cv2.INTER_CUBIC)
...
#        break
#load test images
test_img1 = cv2.imread("test-data/obama.png")
#test_img2 = cv2.imread("test-data/my-pic.png")
